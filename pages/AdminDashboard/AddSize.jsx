...
import { addCategoryServer, deleteSizeServer } from "../../lib/categoryActions";
...
const AddSize = () => {
  ...
  const onSubmit = async (data) => {
    const key = "productSize";
    const value = data.productSize;
    try {
      ...
      <div className="flex flex-col items-center gap-4 p-4 bg-white shadow-lg rounded-md">
        ...
        <div className="w-full flex justify-end gap-3">
          ...
          <button
            onClick={async () => {
              try {
                await deleteSizeServer(size.id);
      ...
      <section className="mt-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6  gap-4">
          {productSize.map((size, index) => (
            ...
            >
              <span className="text-gray-800 font-semibold uppercase text-base">
                {size.value}
              </span>
              <button
